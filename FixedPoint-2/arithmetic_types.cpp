...
template <typename T> void Computation1a(const char* p, std::ostream& os)
{
	T res{ 0 }, g(9.8), v0(5), d0(50);
...
template <typename T> DWORD Computation1b(const char* p, int iterations, std::ostream& os)
{
	T res{0}, g(9.8), v0(5), d0(50);
	...
	// std::chrono::duration<double, std::ratio<1,1'000>> diff = end - start;

	// auto int_rounded = std::chrono::round<std::chrono::milliseconds>(end - start);
	auto int_rounded = std::chrono::round<std::chrono::duration<unsigned long, std::milli>>(end - start);

	// auto int_ms = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);

	if( int_rounded.count() >= TEST_GRANULE)
	{
		os << p << iterations << " iterations in " << int_rounded.count() << "mS" << std::endl;
	}
	// if (tick >= TEST_GRANULE)
	// 	os << p << iterations << " iterations in " << tick << "mS" << std::endl;
	// return tick;
	return int_rounded.count();
}// end Computation1b

//	 test rounding algorithm by performing typical rounding computation
template <typename T> void RoundingTest(const char* p, std::ostream& os)
{
...
template <typename T> void OverflowTest(const char* p, std::ostream& os)
{
...
//	test driver
inline int arithmetic_types_module_test(int nTest, std::ostream& os)
{
	bool rc = true;
	...
	case 12:	//	typical computation using fixed binary with a different policy
		{
			static const int bits = 10;
			const char* p = "fixed_binary<10, long, policy_restrict_range> ";
	...
	case 15://	typical computation using  unsigned fixed binary
		{
			static const int bits = 10;
			const char* p = "unsigned_fixed_binary<10> ";
	...
	case 11:	//	typical computation using fixed decimal with a different policy
		{
			...
			const char* p = "fixed_decimal<3, long, policy_restrict_range> ";
			os << "\ntest " << nTest << ": " << p << std::endl;
	...
	case 14://	typical computation using  unsigned fixed decimal
		{
			...
			const char* p = "fixed_decimal<3, unsigned long> ";
			os << "\ntest " << nTest << ": " << p << std::endl;
		//	RoundingTest <fixd>(p, os);
			os << p << ": rounding test" << std::endl;
