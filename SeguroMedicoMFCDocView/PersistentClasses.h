...
struct Claim
{
	...
	[[nodiscard]] Claim(int id, int fkey_patient, int fkey_doctor, int fkey_medication,
	                    const std::chrono::sys_days& start_date, const std::chrono::sys_days& submission_date,
	                    const std::string& asprose_claim_number, const std::string& asprose_case_number,
	                    double asprose_amount_presented, const std::string& ins_claim_number, int asprose_acknowledgement_type,
	                    const std::string& comment, double amount, const std::optional<int>& other_system_id, int status)
		: id(id),
		  fkey_patient(fkey_patient),
		  fkey_doctor(fkey_doctor),
		  fkey_medication(fkey_medication),
		  start_date(start_date),
		  submission_date(submission_date),
		  asprose_claim_number(asprose_claim_number),
		  asprose_case_number(asprose_case_number),
		  asprose_amount_presented(asprose_amount_presented),
		  ins_claim_number(ins_claim_number),
		  asprose_acknowledgement_type(asprose_acknowledgement_type),
		  comment(comment),
		  amount(amount),
