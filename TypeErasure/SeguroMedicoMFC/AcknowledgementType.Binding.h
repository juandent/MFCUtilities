...
std::string AcknowledgementTypeToString(AcknowledgementType type) {
    switch (type) {
    case AcknowledgementType::Full:
        return "full";
    case AcknowledgementType::Minimum:
        return "minimum";
    case AcknowledgementType::None:
        return "none";
    }
    throw std::domain_error("Invalid AcknowledgementType enum");
}
 ...
 */
std::unique_ptr<AcknowledgementType> AcknowledgementTypeFromString(const std::string& s) {
    if (s == "full") {
        return std::make_unique<AcknowledgementType>(AcknowledgementType::Full);
    }
    else if (s == "minimum") {
        return std::make_unique<AcknowledgementType>(AcknowledgementType::Minimum);
    }
    else if (s == "none")
    {
        return std::make_unique<AcknowledgementType>(AcknowledgementType::None);
    }
    return nullptr;
}
...
namespace sqlite_orm {
     ...
     */
    template<>
    struct type_printer<AcknowledgementType> : public text_printer {};
     ...
     */
    template<>
    struct statement_binder<AcknowledgementType> {

        int bind(sqlite3_stmt* stmt, int index, const AcknowledgementType& value) {
            return statement_binder<std::string>().bind(stmt, index, AcknowledgementTypeToString(value));
     ...
     */
    template<>
    struct field_printer<AcknowledgementType> {
        std::string operator()(const AcknowledgementType& t) const {
            return AcknowledgementTypeToString(t);
        }
    };
     ...
     */
    template<>
    struct row_extractor<AcknowledgementType> {
        AcknowledgementType extract(const char* row_value) {
            if (auto type = AcknowledgementTypeFromString(row_value)) {
                return *type;
            }
            else {
                throw std::runtime_error("incorrect AcknowledgementType string (" + std::string(row_value) + ")");
            }
        }

        AcknowledgementType extract(sqlite3_stmt* stmt, int columnIndex) {
