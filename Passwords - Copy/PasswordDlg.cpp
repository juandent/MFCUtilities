...
BOOL PasswordDlg::OnInitDialog()
{
	...
	m_location_namesCB.loadBox(true, order_by(&Location::name));
...
void PasswordDlg::OnCbnSelchangeCbLocationNames()
{
	...
	m_password_listLB.loadBox(where_clause, order_by(&Password::begining_date).desc());
...
{
	// TODO: Add your control notification handler code here
	auto password = getCurrent<Password>(m_id, storage);
	auto location = m_location_namesCB.current();
