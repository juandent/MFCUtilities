...
BOOL LocationDlg::OnInitDialog()
{
	...
	// storage
	// auto vec = storage.get_all<Location>(order_by(&Location::name));
	// m_list_all_locationsLB.moveVectorIntoBox(vec);
	m_list_all_locationsLB.loadBox(true, order_by(&Location::name));
...
void LocationDlg::OnBnClickedApply()
{
	...
	auto location = getCurrent<Location>(m_id, storage);
